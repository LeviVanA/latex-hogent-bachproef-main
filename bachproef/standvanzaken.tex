...
Dit onderzoek maakt gebruik van inzichten uit een recente publicatie in MDPI Electronics, dat een uitgebreide analyse van cybersecuritydreigingen beschrijft en 
de effectiviteit van verschillende verdedigingsmechanismen binnen webapplicaties biedt. Een bijkomend doel is dan ook om de kennis uit te diepen hoe webapplicaties beter 
beschermd kunnen worden tegen geavanceerde cyberaanvallen. Deze publicatie benadrukt specifiek het belang van voortdurende vernieuwing 
in beveiligingsstrategieën om voldoende in te spelen op de snel evoluerende dreiging van cyberaanvallen ~\autocite{Altulaihan2023}.

\section{\IfLanguageName{dutch}{Pentesting}{Pentesting}}
\label{sec:pentesting}
Penetration testing (of pentesting) vormt een cruciaal onderdeel bij het waarborgen van netwerk-, systeem- en applicatiebeveiliging. 
Wetenschappelijke studies tonen aan dat het proces van pentesting verschillende methodieken omvat, zoals white box, 
black box en grey box tests, elk met hun eigen aanpak van kwetsbaarheden in een systeem. Het doel van pentesting is om 
...
Tijdens de voorbereidingsfase wordt Nmap (een pentesttool) gebruikt om de netwerkstructuren te verkennen en de actieve hosts 
(printers, computers, routers...) en services (HTTP, FTP, SSH...) te identificeren op het netwerk. Dit helpt bij het 
vaststellen van de scope en de doelstellingen van de penetratietest door het aanvalsoppervlak duidelijk te maken. 

Vervolgens vindt in de uitvoeringsfase een test plaats, waarbij tools en methoden worden 
...
Metasploit (nog een andere soort pentesttool) wordt daarop gebruikt om deze 
kwetsbaarheden te exploiteren. Hiermee evalueren testers of ze daadwerkelijk toegang kunnen krijgen tot 
...
Bij het selecteren van tools en methoden voor penetratietests moet rekening worden gehouden met verschillende factoren, zoals 
de grootte van het netwerk, het soort infrastructuur en het type kwetsbaarheden die worden getest. Wetenschappelijke studies 
...
wordt uitgevoerd ~\autocite{Alhamed2023}. Naast deze overwegingen is het ook essentieel om specifieke normen en richtlijnen 
te volgen, zoals ISO 27000, die ethische aspecten en best practices voor pentesting benadrukken 
~\autocite{DalalanaBertoglio2017}.

Recente technologische ontwikkelingen, zoals deep reinforcement learning\footnote{Bij deep reinforcement 
learning wordt de patroonherkenning van deep learning gecombineerd met neurale 
...
taken uitvoeren door te leren van hun interacties met de omgeving}, kunnen een belangrijke rol spelen in het verder verbeteren van pentestingmethoden. 
Deze nieuwe methodieken kunnen de efficiëntie van het proces verhogen en bijdragen aan een betere detectie van kwetsbaarheden 
~\autocite{Yi2023}.

Voor gedetailleerde informatie en diepgaande analyses van penetratietests in verschillende contexten worden wetenschappelijke 
artikelen en studies door pentesters geëvalueerd, die het onderwerp uitvoerig behandelen ~\autocite{Sarker2023}.
...
Bij het testen van webomgevingen met name het CMS WordPress en php framework Laravel, speelt het identificeren van veelvoorkomende 
kwetsbaarheden een cruciale rol, aangezien exploits bij een bekend framework als WordPress onmiddelijk verspreid worden aan een 
hoog tempo. Zoals eerder aangehaald valt het onderzoeken van kwetsbaarheden in webapplicaties onder 
de fase voorbereiding van een pentest. Bij de testen die later worden uitgevoerd, wordt een onderscheid gemaakt tussen een 
Laravel applicatie framework en een WordPress CMS framework, waarbij deze laatste nog wordt onderscheiden door een 
installatie met en zonder beveiligingsplugins.

In een WordPress omgeving zonder beveiligingsplugins worden misconfiguraties zoals onbeschermde back-ups 
en tijdelijke bestanden geïdentificeerd, omdat ze gevoelige informatie kunnen lekken. Deze onbeschermde bestanden kunnen 
...
Bij het testen van WordPress met beveiligingsplugin(s) is het belangrijk om de effectiviteit van beveiligingsmaatregelen van de geïnstalleerde plugin te 
...
Daarnaast is het essentieel om te testen op SQL-injecties en te zorgen voor veilige authenticatie in dergelijke applicaties 
...
veiligere softwarepraktijken. Ze zijn een autoriteit door hun publicatie van de OWASP Top 10, een lijst die de tien meest kritieke veiligheidsrisico's 
benadrukt. Deze lijst wordt breed erkend en gerespecteerd in de cybersecuritygemeenschap en dient als een fundamentele leidraad 
voor ontwikkelaars en beveiligingsexperts wereldwijd. 

OWASP helpt organisaties hiermee bij het effectief beschermen van hun digitale 
...
~\autocite{Priyawati2022}, die hierna worden toegelicht.

\begin{figure}
    \centering
    \includegraphics[height=0.2\textheight]{sql-injection_schema.png}
    \caption[Visualisatie van een SQL-injection aanval]{Visualisatie van een SQL-injection aanval ~\autocite{Biswas2023}}
\end{figure}
\subsubsection{\IfLanguageName{dutch}{SQL-injectie}{SQL-injection}}
\label{sec:SQL-injectie}

SQL-injectie is een veelvoorkomende cyberbeveiligingsdreiging waarbij aanvallers schadelijke SQL-code injecteren in webapplicaties met als doel om databases te manipuleren. 
Het risico omvat het stelen van gegevens, het wijzigen van records of zelfs het verwijderen van belangrijke informatie. Om dit te bestrijden, wordt inputvalidatie 
...
injectie wordt verkleind. Inbraakdetectiesystemen spelen een cruciale rol in de beveiliging. Dergelijke systemen monitoren netwerkverkeer op abnormale patronen die wijzen op 
een inbraakpoging, zoals ongewoon veel database verzoeken. Deze technieken verhogen de weerbaarheid van systemen tegen SQL-injectie aanvallen, elk 
...
\begin{figure}
    ...
    \caption[Visualisatie van een XSS-aanval]{Visualisatie van een XSS-aanval ~\autocite{Ilyin2024}}
\end{figure}

Cross-Site Scripting (XSS) is een type cyberaanval waarbij aanvallers kwaadaardige scripts op websites plaatsen, die vervolgens worden uitgevoerd via de 
browsers van bezoekers van de site. Dit kan leiden tot gestolen cookies, waarbij aanvallers toegang krijgen tot opgeslagen sessiegegevens die authenticatie mogelijk 
maken. Sessiehijacking houdt in dat aanvallers de controle over de gebruikerssessie overnemen. 

Ook kan er manipulatie van de website-inhoud plaatsvinden, 
waardoor gebruikers misleidende informatie zien. Content Security Policies (CSP) zijn cruciale verdedigingsmaatregelen die bepalend welke contentbronnen 
...
Broken Access Control (BAC) is een kwetsbaarheid in webapplicaties die ontstaat wanneer de websites niet effectief de gebruikersactiviteiten beperken. 
Dit kan leiden tot situaties waarbij gebruikers acties kunnen uitvoeren waarvoor ze geen toestemming hebben. De risico's omvatten toegang tot gevoelige data, het wijzigen 
...
pogingen zijn toegepast. Hackers voeren deze aanvallen uit met verschillende motieven, waaronder het stelen van persoonlijke of financiële informatie, het uitvoeren van 
frauduleuze transacties of het saboteren van systemen. Dit type aanval benadrukt het belang van sterke wachtwoordbeleid en beveiligingsmaatregelen om dergelijke aanvallen te weerstaan
...
vaak anti-CSRF tokens, die moeten worden meegezonden met elk verzoek. Zonder het juiste token wordt het verzoek geweigerd, wat helpt om de 
gebruiker te beschermen tegen dergelijke ongewilde acties ~\autocite{B.I.2024}.

\subsubsection{\IfLanguageName{dutch}{Onvoldoende logging en monitoring}{Insufficient logging and monitoring}}
...
vertraagde detectie van inbreuken, onvolledige forensische analyses na een incident of het niet herkennen en reageren op 
...
\subsubsection{\IfLanguageName{dutch}{Sensitive data exposure}{Sensitive data exposure}}
...
of persoonlijke identificatiegegevens, onbedoeld wordt blootgesteld aan onbevoegde personen. Dit kan zich bijvoorbeeld 
voordoen bij het verzenden van gegevens via onbeveiligde kanalen (zoals HTTP in plaats van HTTPS), onjuiste opslag van 
gegevens zonder adequate encryptie, of door fouten in de configuratie van toegangscontroles. Deze kwetsbaarheid kan leiden 
tot ernstige gevolgen, waaronder identiteitsdiefstal, financiële fraude, reputatieschade voor bedrijven of juridische sancties 
...
\begin{figure}
    ...
    \caption[Aantal data breaches in de US ]{Aantal data breaches in de US ~\autocite{Petrosyan2024}}
...
In dit onderdeel worden de security mechanismen van de verschillende webomgevingen uit dit onderzoek verder geanalyseerd. Elk 
platform of framework heeft zijn eigen beveiligingsmaatregelen die bijdragen aan de algehele veiligheid van webapplicaties.

WordPress, een veelgebruikt content managementsysteem, bevat enkele ingebouwde beveiligingsfuncties zoals gebruikersrollen, beveiligde 
...
De piek in datalekken in 2016 die teruggevonden kan worden op figuur \ref{fig:data-breaches} is grotendeels te wijten aan enkele grootschalige problemen, zoals het Yahoo-datalek waarbij 
...
Bij het kiezen van de juiste pentesting tool voor webomgevingen moet worden overwogen welke functionaliteiten 
elke tool biedt. Dit is van cruciaal belang, gezien de vele verschillende types webapplicaties en ontwikkelplatformen en de 
bijhorende veiligheidsrisico's.
...
is dan ook niet onlogisch om voor verschillende testen verschillende tools te gebruiken ~\autocite{Deepikakongara2023}.
Zoals aangehaald in het eerste hoofdstuk \nameref{sec:pentesting}, is het belangrijk om eveneens rekening te houden met de grootte van het netwerk, 
...
Een analyse, toegankelijk via Google Books, een volledige vergelijking tussen enkele van de meest gekende pentesting tools op de markt, 
zoals Nmap, Burp Suite, en OWASP ZAP. Deze bron is bijzonder waardevol voor professionals binnen de cybersecuritywereld, omdat ze inzicht geeft in de sterke en zwakke 
punten van elk van deze tools. Door ze objectief te vergelijken, kunnen teams betere beslissingen nemen over welke het meest geschikt zijn 
...
voor zowel automatische als zelf ontwikkelde handmatige tests, wat de flexibiliteit vergroot en het geschikt maakt voor zowel beginners als gevorderde gebruikers. 
De voortdurende updates en de mogelijkheid om modules of andere tools zoals Nmap en Wireshark te integreren maken Metasploit tot een handige tool voor elke 
...
\section{\IfLanguageName{dutch}{Wat is geweten binnen de literatuur}{What is known from the literature}}
...
selecteren van effectieve tools is van belang om kwetsbaarheden in webapplicaties te identificeren en aan te pakken. 
Deze selectie vereist niet alleen technische kennis, maar ook strategisch inzicht in hoe verschillende tools diverse 
...
beoordelen en bijwerken, zodat ze voorbereid zijn op zowel de huidige als toekomstige cyberuitdagingen. Dit continu 
...
Een belangrijk aspect van dit onderzoek behelst de impact van beveiligingsplugins op WordPress-websites. Deze plugins, zoals Wordfence en iThemes Security, worden 
...
te weren, waardoor ze een belangrijk onderdeel vormen van de beveiligingsstrategie voor elke WordPress-omgeving~\autocite{Casola2020}.
...
Als het CMS-systeem WordPress vergeleken wordt met het Laravel-framework, valt op dat dit laatste beveiligingsvoordelen biedt. Deze voordelen worden voornamelijk toegeschreven aan 
...
\section{\IfLanguageName{dutch}{Wat weten is niet geweten binnen de literatuur?}{What is not known from the literature?}}
...
focussen op de pijnpunten binnen de cybersecurity zoals de menselijke factor en de complexiteit van beveiligingsmaatregelen. Menselijke fouten vormen immers een belangrijk 
deel van de zwakten van cyberbeveiliging. Het kan zo bijvoorbeeld enorm moeilijk zijn om de juiste systeemconfiguratie te beheren, ook als daarvoor grote IT-teams worden 
...
afweren van cyberdreigingen. Dit draagt dan op zijn beurt bij tot een veiligere digitale omgeving, waarbij de focus ligt op preventie en effectieve respons
...
over de methoden die deze tools gebruiken om kwetsbaarheden te identificeren en te analyseren. Dit veronderstelt een breder begrip van de dynamiek van pentesting, waarmee 
...
om bestaande theoretische kaders te evalueren. En het draagt bovendien bij tot de ontwikkeling van vernieuwde theorieën die de complexe problemen van hedendaagse 
...
stimuleert het onderzoek de evolutie van theoretische benaderingen in cybersecurity. Tegelijkertijd levert het belangrijke inzichten voor het implementeren van beveiligingsmaatregelen
die overeenkomen met de realiteit van de huidige digitale computercriminaliteit.
Dit proces helpt het verfijnen en versterken van webbeveiligingspraktijken, gericht op het voldoende beschermen tegen en reageren op moderne 
...
De maatschappelijke relevantie van dit onderzoek is dan ook veelzijdig. Het draagt direct bij tot het verbeteren van de digitale veiligheid en privacy van personen 
